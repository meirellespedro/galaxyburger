...
const INVENTORY_KV_KEY = "hamburgeria:inventory-status";
...
async function persistInventoryStateToKv(state) {
  await kvSetJSON(INVENTORY_KV_KEY, state);
}

async function readInventoryStateFromKv() {
  const rawState = await kvGetJSON(INVENTORY_KV_KEY);
  const { state, didChange } = ensureInventoryStateShape(rawState);

  if (didChange) {
    await persistInventoryStateToKv(state);
  }

  return {
    state,
    ...buildInventoryStorageMeta("kv", true)
  };
}
...
async function readInventoryState() {
  ...
  if (storageMode === "kv") {
    return readInventoryStateFromKv();
  }
