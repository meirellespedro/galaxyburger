...
async function persistInventoryStateToBlob(state) {
  const pathname = resolveInventoryBlobPathname();
  ...
  try {
    ...
    throw createBlobStorageAccessError(error, {
      createStorageError: createInventoryStorageError,
      codePrefix: "inventory",
      operation: "write",
      pathname,
      fallbackMessage: "Nao foi possivel salvar o estoque no Vercel Blob agora."
    });
  }
}
...
async function readInventoryStateFromBlob() {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Handle the response correctly: it can be a ReadableStream or similar
      // The response object should have a stream property
      if (!response || typeof response.stream === "undefined") {
        const initialState = buildDefaultInventoryState();
        logBlobStorageEvent("warn", "inventory_blob_missing_initializing", {
          pathname,
      ...
      if (attempt < maxRetries) {
        logBlobStorageEvent("warn", "inventory_blob_read_retry", {
          attempt,
          maxRetries,
          pathname,
          error: sanitizeBlobError(error)
  ...
  throw createBlobStorageAccessError(lastError, {
    ...
    operation: "read",
    pathname,
    fallbackMessage: "Nao foi possivel carregar o estoque do Vercel Blob agora."
  });
}
...
async function readInventoryState() {
  ...
  if (storageMode === "blob") {
    try {
      return await readInventoryStateFromBlob();
    } catch (error) {
      const isAuthError = Number(error?.statusCode || 0) === 503 && normalizeText(error?.code).includes("forbidden");
      if (isAuthError) {
        logBlobStorageEvent("warn", "inventory_blob_auth_fallback", { error: sanitizeBlobError(error) });
        return {
