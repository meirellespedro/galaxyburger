...
const STORE_STATUS_KV_KEY = "hamburgeria:store-status";
...
async function persistStoreStatusStateToKv(state) {
  await kvSetJSON(STORE_STATUS_KV_KEY, state);
}

async function readStoreStatusStateFromKv() {
  const rawState = await kvGetJSON(STORE_STATUS_KV_KEY);
  const { state, didChange } = ensureStoreStatusStateShape(rawState);

  if (didChange) {
    await persistStoreStatusStateToKv(state);
  }

  return {
    state,
    ...buildStoreStatusStorageMeta("kv", true)
  };
}
...
async function readStoreStatusState() {
  ...
  if (storageMode === "kv") {
    return readStoreStatusStateFromKv();
  }
...
async function persistStoreStatusState(state, storageMode = resolvePreferredStoreStatusStorageMode()) {
  if (storageMode === "readonly") {
    throw createStoreStatusStorageError(
      ...
      "O controle de abertura da loja esta em modo somente leitura neste ambiente.",
      503
    );
  }
